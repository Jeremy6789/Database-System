...
def delete_selected():
    """
    作業4新增功能：處理透過 Checkbox 選取的多筆刪除請求
    """
    try:
        # 1. 從表單中獲取所有被勾選的 checkbox 值 (name="selected_ids")
        # getlist 會回傳一個 list，包含所有被勾選的 value
        ...
        # 2. 將字串 ID 轉換為 MongoDB 的 ObjectId
        object_ids = [ObjectId(id) for id in selected_ids]
        
        # 3. 使用 MongoDB 的 $in 運算子進行批次刪除
        # delete_many({ "_id": { "$in": [id1, id2, id3...] } })
        result = employees_collection.delete_many({"_id": {"$in": object_ids}})
        
        flash(f"Successfully deleted {result.deleted_count} selected employees!", "success")
        
    except Exception as e:
        flash(f"An error occurred during multiple delete: {e}", "error")
